...
rm ParallelForest_1.1.1.tar.gz 
...
if [ "$1" != "devel" ]; then
	...
	R CMD check --as-cran ParallelForest_1.1.1.tar.gz
else
	echo Build, install, and check package for development version of R...
	R-devel CMD build ParallelForest
	R-devel CMD INSTALL ParallelForest_1.1.1.tar.gz
	R-devel CMD check --as-cran ParallelForest_1.1.1.tar.gz
fi
